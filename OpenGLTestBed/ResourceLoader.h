...
public:
	...
	static void LoadSceneFromAIScene(const aiScene* aiscene, Scene* scene)
	{
      ...
      if (aiscene->HasMeshes())
      {
			...
			{   
				...
				// Size the arrays
            std::vector<float3> vertices = std::vector<float3>(aiscene->mMeshes[i]->mNumVertices);
            std::vector<float3> normals = std::vector<float3>(aiscene->mMeshes[i]->mNumVertices);
				std::vector<float2> texCoords = std::vector<float2>(aiscene->mMeshes[i]->mNumVertices);

				// Get the vertices, vertex normals 
            for (UINT j = 0; j < aiscene->mMeshes[i]->mNumVertices; j++)
            {
               // Vertices
               vertices[j] = float3(aiscene->mMeshes[i]->mVertices[j].x,
                  aiscene->mMeshes[i]->mVertices[j].y,
                  aiscene->mMeshes[i]->mVertices[j].z);

               // Normals
               normals[j] = float3(aiscene->mMeshes[i]->mNormals[j].x,
               ...
               if (mesh.mat->hasDiffuseMap && aiscene->mMeshes[i]->HasTextureCoords(0))
               {
                  ...
                  texCoords[j] = float2(u, v) * mesh.mat->diffuseMap->GetDim();
               }
            }

				// Indices
				std::vector<UINT> indices = std::vector<UINT>(aiscene->mMeshes[i]->mNumFaces * 3);
            ...
            mesh.SetVertexBuffer(vertices, normals, texCoords);
   ...
   // Loads the entire directory as one shader
   ...
   static GLuint LoadShaderFromPath(const std::string path, GLenum shaderType)
   {
